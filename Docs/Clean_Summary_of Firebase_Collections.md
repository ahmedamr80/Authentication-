...
**🔥 High-Level Summary ofYour Firestore Collections**
...
**Not tied to a specific event —used everywhere else (registrations, teams)**
...
| **coordinates**         | lookup field from the clubs’ collection by matching the clubId and fetching the coordinates data from the respective club |
...
| **locationName**        | lookup field from the clubs’ collection by matching the clubId and fetching the club name data from the respective club   |
| **logoUrl**             | photo url                                                                                                                 |
...
  2. System checks `slotsAvailable` vs `confirmedUnits`.

  3. If Team Mode: check point 4.3

  4. Outcome:

    **CONFIRMED:** Slot secured.

    **WAITLIST:** Added to FIFO queue with a specific `waitlistPosition`.

**Withdrawal & Promotion:**

  * User withdraws → Slot opens.

  * System automatically promotes the first eligible waitlist entry(First-In-First-Out).

  * Promoted player is notified.
...
  *** Create registration with `partnerStatus:'PENDING'`.**

  *** Display Player A as **Confirmed**(Initiator).**

  *** Display Player B as **PendingConfirmation**.**

  *** **Notification:** Send notification toPlayer B: *"Player A has chosen you as a partner. Confirm or Deny?"***

**3. **Confirmation:** Player Baccepts.**

**4. **Result:****

  *** Update `partnerStatus` to `'CONFIRMED'`.**

  *** **Notification:** Send notification toPlayer A: *"Player B accepted."***

  *** **Count:** Increment Confirmed Teams count(+1).**
...
  *** Update `partnerStatus` to `'DENIED'`.**

  *** **Notification:** Send notification toPlayer A: *"Player B denied your request."***

  *** **Count:** Team is **NOT** counted.Registration is effectively invalid until Player A selects a new partner andplayer A is moved to **"Single Players"**.**
...
  *** Player A is added to the **"SinglePlayers"** section of the event.**

  *** `partnerStatus` is `'NONE'`.**

  *** Player A is visible to other Singles forteaming up.**
...
  *** **Notification:** Immediately notify PlayerB: *"Player A cancelled the registration."***

  *** **Data Update:****

    *** Remove the "Team" entity.**

    *** Move Player B from "ConfirmedTeams" list to "Single Players" (or "Pending" state).**

    *** Update Player B's record to `partnerStatus:'NONE'` (or similar).**

  *** **Count:** Decrement Confirmed Teams count(-1).**
...
·         1 from the initiatorP1RegId

·         1 from the responderP2RegId
...
**📊 Relationship Table(Simple & Clear)**

| **Collection** | **Linked To** | **Relationship Type**           | **Notes**                                                                                          |
| -------------- | ------------- | ------------------------------- | -------------------------------------------------------------------------------------------------- |
| users          | registrations | 1 user → many registrations     | Via playerId (but only 1 per Event)                                                                |
| users          | teams         | 1 user → many teams             | As player1Id/player2Id                                                                             |
| users          | events        | 1 user → 1 events               | Via uid                                                                                            |
| clubs          | events        | 1 club → many events            | Via clubId                                                                                         |
| events         | registrations | 1 event → many registrations    | Via eventId                                                                                        |
| events         | teams         | 1 event → many teams            | Via eventId                                                                                        |
| teams          | users         | many-to-many                    | 2 users per team                                                                                   |
| teams          | registrations | 1 team → 2 registrations(Teams) | unitType = "Teams"<br> P1RegId = id of player 1 registration P2RegId = id of player 2 registration |
...
  > "Generate a short admin-ready summary (2-3 lines) for:{{eventName}}, Date: {{dateTime}}, Slots: {{slotsAvailable}} {{unitType}}.Tone: Concise, suitable for email."

2. **Waitlist Promotion:**

  > "Write a short urgent message: Event {{eventName}}. You arepromoted. Confirm within 10 mins. Tone: Urgent."

3. **Cancellation:**

  > "Draft a polite cancellation message for {{eventName}}.Reason: {{cancellationMessage}}. Tone: Apologetic."

4. **Partner InviteNotification:**

  > "Trigger: New Team Reg. Prompt: 'Player {{playerAName}} wantsto team up with you for {{eventName}} on {{date}}. Action: Confirm or Deny inapp.'"

5. **Partner RejectionNotification:**

  > "Trigger: Partner Deny. Prompt: '{{playerBName}} declined yourteam invite for {{eventName}}. You are now listed as a single player. Visit theapp to find a new partner.'"

6. **Team CancellationNotification:**

  > "Trigger: Cancellation (Scen 4). Prompt: 'Urgent: Your partner{{playerAName}} cancelled their registration for {{eventName}}. You have beenmoved to the Single Players list. Please find a new partner to secure your teamslot.'"
